...
def find_diagnal_index_with_face_vectorized(
    face, iy, ix, tp, xoffset=1, yoffset=1, moves=[0, 3], cuvwg="G"
):
    ...
    for j in redo:
        ...
        try:
            nface[j], niy[j], nix[j] = tp.ind_moves(
                (face[j], iy[j], ix[j]), singleuse_moves, cuvwg=cuvwg
            )
        except IndexError:
...
def sd_position_from_latlon(lat, lon, ocedata):
    ...
    (
        ...
        (pt.fcg[upper_left], pt.iyg[upper_left], pt.ixg[upper_left]),
        0 * np.ones(len(upper_left), int),
        cuvwg="G",
    )
    ...
    (
        ...
        (pt.fcg[lower_righ], pt.iyg[lower_righ], pt.ixg[lower_righ]),
        3 * np.ones(len(lower_righ), int),
        cuvwg="G",
    )
    pt.rxg[lower_righ] = pt.rx[lower_righ] - 0.5
    need_rot = np.where(pt.face != pt.fcg)[0]
    for i in need_rot:
        edge, new_edge = tp.mutual_direction(pt.face[i], pt.fcg[i], transitive=True)
        rot = (
            np.pi - sd.topology.DIRECTIONS[edge] + sd.topology.DIRECTIONS[new_edge]
        ) % (np.pi * 2)
        pt.rxg[i], pt.ryg[i] = sd.utils.local_to_latlon(
            pt.rxg[i], pt.ryg[i], np.cos(rot), np.sin(rot)
        )
    return pt
...
def scalar_data_retrieve(pt, scalar_knw=scalar_knw, grain=None, exmp_scl=None):
    ...
    masked = np.logical_or(
        exmp_scl[tuple(inds.T)] == 0, np.isnan(exmp_scl[tuple(inds.T)])
    )
...
def vort_data_retrieve_with_face(pt, grain=None, exmp_vel=None):
    ...
    masked = np.logical_or(
        exmp_vel[tuple(inds.T)] == 0, np.isnan(exmp_vel[tuple(inds.T)])
    )
...
def weight_index_inverse_from_latlon(
    oce, lat, lon, var="scalar", grain=None, exmp_vel=None, exmp_scl=None
):
    ...
    if var == "scalar":
        ...
        ind, inverse, (rotu, rotv) = vort_data_retrieve_with_face(
            pt, grain=grain, exmp_vel=exmp_vel
        )
        ...
        return (
            (du_weight.astype("float32"), dv_weight.astype("float32")),
            ind.astype("int32"),
            (inverse.astype("int32"), splitter),
        )


def store_interpolator(
    env, zooms, subocedata, unique_grains, inverse_grain, exmp_vel=None, exmp_scl=None
):  # pragma: no cover
            ...
            for blck_x in range(number_of_blocks):
                for blck_y in range(number_of_blocks):
                    for var in ["scalar", "vort"]:
                        ...
                        interpolator = weight_index_inverse_from_latlon(
                            sub2use,
                            y,
                            x,
                            var=var,
