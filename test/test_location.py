...
def example_fields():
    ...
    uv = np.array(
        da.concatenate(
            [ds.UVELMASS[0, 0].data[np.newaxis], ds.VVELMASS[0, 0].data[np.newaxis]],
            axis=0,
        )
    )
    return s, uv
...
@pytest.mark.parametrize(
    "face,iy,ix,ans", [(10, 89, 45, (2, 44, 0)), (5, 45, 45, (5, 46, 46))]
)
...
def test_find_diagnal_index_intervene(od, face, iy, ix, ans):
    tp = od.tp
    res = find_diagnal_index_with_face_vectorized(
        face, iy, ix, tp, xoffset=-1, yoffset=-1, moves=[1, 2], cuvwg="C"
    )
    res = np.array(res)
...
def test_weight_index_inverse_from_latlon(od, xy, example_fields, var, grain):
    ...
    weight_index_inverse_from_latlon(
        od, lat, lon, var=var, grain=grain, exmp_vel=exmp_vel, exmp_scl=exmp_scl
    )
